...
int Graph::RemoveVertex(int id){
  ...
  for (int i = G[id].begin(); i != G[id].end(); i = G[id].GetNext(i)) {
    ...
    if(*vlist.GetNext(start + deg + n) > n)
      deg = *vlist.GetPrev(start + GetNext(deg + n)) - n;
...
int Graph::GetNext(int id){
  return (n <= *vlist.GetNext(vlist.begin() + id) and *vlist.GetNext(vlist.begin() + id) < n + org_deg + 1)?
    GetNext(*vlist.GetNext(vlist.begin() + id)):
    *vlist.GetNext(vlist.begin() + id);};

int Graph::GetPrev(int id){
  return (n <= *vlist.GetPrev(vlist.begin() + id) and *vlist.GetPrev(vlist.begin() + id) < n + org_deg + 1)?
    GetPrev(*vlist.GetPrev(vlist.begin() + id)):*vlist.GetPrev(vlist.begin() + id);};
