...
public:
  ...
  inline std::unique_ptr<T>& GetNext(std::unique_ptr<T>& pos){return list[next[pos - list]];};
  inline std::unique_ptr<T>& GetPrev(std::unique_ptr<T>& pos){return list[prev[pos - list]];};
